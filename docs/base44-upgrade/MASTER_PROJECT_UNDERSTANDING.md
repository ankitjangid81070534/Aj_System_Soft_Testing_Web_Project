...
**Updated acceptance:** [visible acceptance report](PHASE_4_30_VISIBLE_ACCEPTANCE.md) closes the scoped public foundation checkpoint: mobile Home/quote screenshots reviewed, real enquiry/field-clear/return gestures pass without new errors; six fresh independent journeys and 460 tests/type/lint pass. The initial stream errors did not recur; no fix claimed. Current hydration diff identifies browser-injected body attributes, with owner cleanup and ad-load failure still outstanding. Wider capture was 919×499, not desktop; later wider gesture timed out with hidden preview. No full desktop/dark/private visual or backend/release sign-off.

## Continuation

See [route/features](ROUTE_FEATURE_MATRIX.md), [admin](ADMIN_FEATURE_MATRIX.md), [regression](REGRESSION_BASELINE.md), [SEO](SEO_BASELINE.md), [design](DESIGN_BASELINE.md), [decisions](DECISIONS.md), [risks](RISKS.md). **STOP after scoped Phase 4 completion; await a new exact continuation before Phase 5.** Earlier hidden-preview gates are historical; current acceptance limits and retained follow-ups are in the visible acceptance report. Current live Home order remains unchanged; material moves require separate approval. No PR, merge, production mutation or deployment.
