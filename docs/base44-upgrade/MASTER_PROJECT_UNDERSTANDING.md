...
**2026-09-14 CMS planning continuation:** Owner requested planning only. See [PHASE_16_CMS_REPAIR_PLAN](PHASE_16_CMS_REPAIR_PLAN.md) for source-confirmed gaps, a gated repair sequence, preservation map, schema/RLS reconciliation and acceptance/rollback checks. Fresh 11 existing scoped tests pass with mocked adapters; no application/database change or hosted verification. Recommended first implementation is Builder error/feedback truthfulness, only after approval. Phase 16 remains blocked; Phase 17 has not started.

**Current checkpoint: Phase 16 PARTIALLY VERIFIED; full system/release sign-off BLOCKED.** Read `PHASE_16_FULL_SYSTEM_REGRESSION.md` and current `PHASE_STATUS.md`. Found/fixed populated-search first-Escape handling through the existing close path; three reusable browser regression tests pass on dev/final production. Fresh 415 unit tests, typecheck/lint/build, 760 initial route-size cases, 86 final production gestures, 48 anonymous route probes, 15 endpoint probes and 35 internal links pass at stated scope. No auth/data/schema/config/dependency/SEO change. Six integration entries absent; hosted workflows, real Home Builder/schema reconciliation, visual/manual/native/private gates remain open. Current iframe More-open and final healthy-root/cleanup checks pass, but legal navigation helpers time out and screenshots are hidden. **STOP at Phase 16 partial verification; Phase 17 has NOT started.** Progress PR is not release approval; no automatic merge/deploy.

**Historical Phase 15 checkpoint: Phase 15 scoped accessibility/security/trust work complete.** Read `PHASE_15_ACCESSIBILITY_SECURITY_TRUST.md` and current `PHASE_STATUS.md`. Existing Privacy/Disclaimer pages now appear inside desktop top navbar and mobile More (including search and CMS-label preservation). Notification region semantics and a measured dark-desktop contrast issue are corrected; no auth/data/SEO/security-header/dependency changes. Fresh 415 tests / 51 files, typecheck/lint, isolated build, 60+9 automated scans and 73 independent gestures pass at their stated scope. Live iframe legal navigation and focus restoration pass; post-change screenshot review is blocked by hidden preview. Manual incomplete-rule review, native/private/hosted-integration gates and owner confirmation of existing operational legal promises remain deferred. **STOP before Phase 16 until START NEXT PHASE SAFELY.** Managed PR-per-phase flow only; no deployment or blanket security/accessibility certification.

**Historical Phase 14 checkpoint: Phase 14 missing branch work recovered, scoped anonymous checks freshly passed; full visual/native-device/private gates deferred.** Read `PHASE_14_RECOVERY.md` and current `PHASE_STATUS.md` first. Starting `49e16e4` matched remote main, but Phase 14 `d5be8b3` remained unique to `origin/system-upgrade-init`; its exact patch is now recovered without branch switching/deletion or a main write. Fresh 405 tests / 49 files, typecheck/lint, isolated production build, 760 route-size cases, 20 independent gestures and live-iframe Home/Privacy footer gestures pass. Screenshots remain unavailable because the preview is hidden; older buffered hydration warning is not diagnosed, though fresh checks add no errors. No credentials/data/auth/SEO/dependency changes or deployment. **STOP before Phase 15. Latest owner instruction supersedes old navbar exclusion: include existing Privacy/Disclaimer in desktop top navigation and mobile More during Phase 15.** Standing managed PR-per-phase flow remains; no automatic branch consolidation or direct main updates.

**Historical checkpoint: Phase 13 scoped performance optimization and local measurement complete.** See `PHASE_13_PERFORMANCE.md` and the top of `PHASE_STATUS.md`. Exact blog projections preserve reading time/authorship, independent requests now run concurrently, and existing below-fold gallery/teaser images have safer loading/size hints. No navbar/layout/CSS, content, routes, auth/RLS/schema or dependency changes. Actual measured homepage LCP is the unanimated server-rendered H1; one self-hosted font is requested. Fresh 403 tests / 48 files, typecheck, lint, isolated production build, 18 controlled homepage runs and six production route responses pass. LCP/long-task timings are mixed and CLS unchanged: no causal homepage speedup, field INP/CWV or production sign-off. Live article/anchor/service/FAQ/home gestures pass; complete visual after-review and real CMS query/media measurements remain deferred. No remote writes or deployment. **Stop before Phase 14 — responsive/all-screen-size audit — until owner continuation. Navbar remains excluded.** Use the standing managed branch/PR flow; never auto-merge or directly edit main. Historical notes below do not override this gate.

Cross-chat recovery at `54ba811` confirmed Phase 7 public UI complete and Phase 8 UI already included in main; authenticated Phase 8 checks remain deferred. All fetched branch tips are ancestors of that HEAD or identical, so no missing branch commits need merging. Fresh 295-test/typecheck/lint/isolated-build results and live-preview limitations are in [CONTINUATION_RECOVERY](CONTINUATION_RECOVERY.md). No application changes or new phase were made in recovery; consult PHASE_STATUS for the next execution gate rather than historical 'next phase' statements below.

Phase 1 source fixes and current verification levels are recorded in [PHASE_1_FUNCTIONAL_AUDIT](PHASE_1_FUNCTIONAL_AUDIT.md). The original Phase 0 snapshot below is retained: dashboard lead access and supported slug redirect matching have since been repaired, and publication permission checks added to generic/Builder saves. Full backend verification remains blocked. The owner explicitly deferred those checks to proceed with documentation-only Phase 2; see [information architecture](PHASE_2_INFORMATION_ARCHITECTURE.md), [visitor journeys](PHASE_2_USER_JOURNEYS.md) and [benchmark evidence](PHASE_2_BENCHMARKS.md). The live composition is unchanged and proposed material moves still require approval. [Phase 3](PHASE_3_DESIGN_SYSTEM.md) subsequently refined shared presentation tokens, contrast, focus, shadows and component classes; 210 regression tests and an isolated production build passed at that phase. [Phase 4](PHASE_4_NAVBAR_HERO.md) subsequently extracted/refined the hero, clarified existing service coverage, preserved accepted navigation and added portal disclosure/focus restoration; 232 tests, 15 independent browser journeys and a final isolated production build pass. Current 919px tablet light/dark captures were reviewed. [Phase 5](PHASE_5_HOMEPAGE_SECTIONS.md) then refined homepage-only section presentation while retaining the entire current sequence and copy: 245 tests, 21 independent browser journeys plus runtime guards, public-proxy navigation and isolated production build pass. Its live iframe health checks pass, but hidden screenshot and navigation-helper limitations required independent visual/journey evidence. [Phase 6](PHASE_6_MOTION.md) subsequently refined finite entrances, reveal ownership, batched motion work and keyboard/short-screen scrolling: 260 tests, nine full scroll cycles, keyboard visibility, existing journeys and an isolated build pass. It preserves settled layout/content and stops before Phase 7. Auth/backend blockers remain deferred. [Phase 7](PHASE_7_PUBLIC_PAGES.md) now unifies public headers and adds responsive native article navigation. The final public-UI review is complete: 282 tests, typecheck/lint and 12 fresh responsive layout/interaction checks pass. Actual tablet service and independent phone/tablet/desktop plus dark tablet captures were visually reviewed; see [final review](PHASE_7_FINAL_REVIEW.md). The earlier isolated production build remains historical evidence, and data-backed release gates remain deferred. [Phase 8](PHASE_8_AUTH_PORTAL.md) subsequently refined auth forms and portal presentation without changing Supabase architecture or data/actions. 294 tests, 50 independent browser checks, typecheck/lint and an isolated production build pass. Public auth captures were reviewed; authenticated portal navigation/profile/persistence remain unverified. A demonstrated preview Server Actions origin mismatch is fixed only for the exact development preview origin. STOPPED before Phase 9.
