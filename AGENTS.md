...
- `MarketingHeader` retains the existing session/portal logic and a non-sticky brand masthead. `BottomNavigation` owns the fixed dock on every public breakpoint and a native More dialog; the old `MobileMenu` is no longer mounted. Older presentation scripts expecting `.glass-strong`, `Open navigation menu`, `#mobile-menu`, or a 1440px desktop-nav breakpoint are stale for navigation assertions.
...
- Existing defect, NOT introduced by the dock: NAV_LINKS already includes `/ai-methods`, but HEAD has no corresponding public page; the destination returns 404. The unchanged link remains in More rather than silently removing or redirecting CMS/navigation content. Do not report all eight destinations as working. Successful authenticated login/account operations remain unverified without Supabase credentials.
