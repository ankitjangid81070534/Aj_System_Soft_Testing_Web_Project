...
- `website/src/components/ui/tubelight-navbar.tsx` uses the existing 1024px desktop breakpoint. The latest desktop-only request replaces orange accents with the website's violet accent token on a white surface, and exposes all CMS header links directly in a two-row grid (brand/actions, then links). `BottomNavigation` now has separately CSS-gated mobile and desktop link groups: desktop has no More button and exposes the existing login/account handler directly; mobile retains its original primary/More order, colors, geometry and dialog. Search and its login/appearance utilities remain available. Route-derived selection survives reload/back; the lamp is hidden below 1024px, shared-layout IDs are instance-scoped, and reduced motion disables its spring.
...
- Latest verification: typecheck, targeted ESLint and 146 tests passed. Ten local responsive/menu/search checks passed (320, 393, 768, 1024, 1214, 1280, 1440, 1920, 2560, and 1100×500), including all eight desktop links, no desktop More, bounds, search/filter/navigation, Ctrl+K, scroll compaction, Escape and focus restoration. Live preview verified direct About navigation and lamp selection, all eight visible links, Client Login open/close, search filtering/close, plus mobile More open/close and Home navigation. Desktop About and mobile About screenshots were visually checked; the optional final Home screenshot was blocked when the preview became hidden. Existing mobile CSS was confirmed unchanged except the new hidden desktop group selector. Earlier local tests additionally covered route back/reload and both motion preferences.
