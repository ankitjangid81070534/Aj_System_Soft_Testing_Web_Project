...
- `MarketingHeader` retains the existing session/portal logic. `BottomNavigation` owns the fixed dock and native More dialog; the old `MobileMenu` is no longer mounted. Below 1024px the original bottom dock and non-sticky brand masthead remain; from 1024px the masthead becomes a hidden layout spacer and the same navigation becomes a top floating mineral/champagne dock. Older presentation scripts expecting `.glass-strong`, `Open navigation menu`, `#mobile-menu`, or a 1440px desktop-nav breakpoint are stale for navigation assertions.
